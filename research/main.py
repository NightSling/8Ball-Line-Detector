...
def mouse_callback(event, x, y, flags, param):
    ...
    if event == cv2.EVENT_LBUTTONDOWN:
        if click_stage == 0:
            ...
            x1, y1 = start_point
            x2, y2 = x, y
            roi_bbox = (min(x1, x2), min(y1, y2), max(
                1, abs(x2-x1)), max(1, abs(y2-y1)))
            click_stage = 0
...
def merge_and_select_top_lines(lines, max_lines=3):
    ...
    for i, (slope_i, intercept_i, length_i) in enumerate(line_params):
        ...
        for j, (slope_j, intercept_j, length_j) in enumerate(line_params):
            if used[j]:
                continue
            if slope_i == np.inf and slope_j == np.inf or (slope_i != np.inf and slope_j != np.inf and abs(slope_i-slope_j) < 0.1):
...
while True:
    ...
    if roi_bbox:
        ...
        lines = cv2.HoughLinesP(
            edges, 1, np.pi/180, thresh, minLineLength=min_len, maxLineGap=max_gap)
        top_lines = merge_and_select_top_lines(lines)

        colors = [(0, 215, 255),  # gold in BGR
                  (255, 0, 0),    # blue
                  (0, 0, 255)]    # red
        ...
        for slope, intercept in top_lines:
            if slope == np.inf:
                ...
                ex1, ey1, ex2, ey2 = 0, intercept, w / \
                    scale, slope*(w/scale)+intercept
                x1, y1 = int(ex1)+x, int(ey1)+y
                x2, y2 = int(ex2)+x, int(ey2)+y

            p1, p2 = (x1, y1), (x2, y2)
            draw_dotted_line(display_frame, p1, p2,
                             (0, 0, 255))  # main red line
            ...
            if show_reflections:
                for start, end in [(p1, p2), (p2, p1)]:
                    ...
                    for _ in range(3):
                        ...
                        draw_dotted_line(
                            display_frame, (cx, cy), (nx, ny), (0, 255, 0))
                        if nx <= x or nx >= x+w:
                            dx = -dx
                        if ny <= y or ny >= y+h:
                            dy = -dy
